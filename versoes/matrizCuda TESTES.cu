...
int main(int argc, char ** argv)
{
	...
	{
		for(int in = 50; in < 3000; in += 50)
		{
			inicializar();
			Input *i = (Input*) malloc(sizeof(Input));
			i->a = gerarMatriz(400, in);
			i->b = gerarMatriz(in, 400);
			i->c = criarMatriz(400, in);
			i->salvar = 0;
	//		printf("Tempo de criacao: %lf\n", medirTempoInput(&i, argc, argv, &lerInput));
			double cpu = medirTempoExecMul(i, multiplicarMatrizesAVX);
			double gpu = medirTempoExecMul(i, multiplicarMatrizesCUDA);
			if(cpu > gpu)
				printf("DEVICE - 400 %d CPU: %lf GPU: %lf\n", in, cpu, gpu);
			else
				printf("HOST   - 400 %d CPU: %lf GPU: %lf\n", in, cpu, gpu);
