...
int main(int argc, char ** argv)
{
	...
	{
		for(int in = 50; in < 2000; in += 50)
		{
			inicializar();
			Input *i = (Input*) malloc(sizeof(Input));
			i->a = gerarMatriz(500, in);
			i->b = gerarMatriz(in, 500);
			i->c = criarMatriz(500, in);
			i->salvar = 0;
	//		printf("Tempo de criacao: %lf\n", medirTempoInput(&i, argc, argv, &lerInput));
			...
			if(cpu > gpu)
				printf("DEVICE - %d %d CPU: %lf GPU: %lf\n", i->c->n, i->c->m, cpu, gpu);
			else
				printf("HOST   - %d %d CPU: %lf GPU: %lf\n", i->c->n, i->c->m, cpu, gpu);
